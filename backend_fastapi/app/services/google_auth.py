...
def verify_google_id_token(id_token_str: str, client_id: str) -> Dict[str, Any]:
    """Decode Google ID token (JWT parsing without network verification)"""
    logger.info(f"🔓 Decoding Google ID token...")
    try:
        # Decode JWT without verification (trust Google's signature from client side)
        # In production, you'd verify the signature, but this is fast and for testing
